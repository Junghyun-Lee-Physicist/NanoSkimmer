...
if __name__ == "__main__":
    ...
    P0+r\P0+r\P0+r\                    default=None, help="Cut string")
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("postproc.log", mode="w")],
    )
    ...
    def run_file(in_file):
        """Process a single input file and report its status."""
        p = PostProcessor(options.outputDir, [in_file],
                          cut=options.cut,
    ...
    total = len(options.inputFile)
    status_map = {}
    for idx, in_file in enumerate(options.inputFile, 1):
        logging.info("Processing file %d/%d: %s", idx, total, in_file)
        try:
            run_file(in_file)
            status_map[in_file] = "finished"
            logging.info("Finished file %s", in_file)
        except Exception as err:
            status_map[in_file] = "failed"
            logging.error("Failed file %s with error: %s", in_file, err)

    logging.info("Processing summary:")
    for f, st in status_map.items():
        logging.info("  %s : %s", f, st)
    if all(st == "finished" for st in status_map.values()):
        logging.info("All files processed successfully.")
    else:
        logging.info("Some files failed. Please check the log for details.")
